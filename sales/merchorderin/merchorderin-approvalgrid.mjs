...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.merchorderin_id)
	header_data = data;
}
...
function grd_list_rowrender(tr) {
	...
	$(tr).find('td').each((i, td) => {
		...
		if (mapping=='docauth_descr') {
			...
			var authby = "";
			var notes = "";			
			if (record.merchorderinappr_isdeclined=="1") {
				gambar = `<img src="images/xtiondecl.png" width="20px" height="20px">`;
				authby = `<div><span style="font-weight: bold">${record.merchorderinappr_declinedby}</span> (${record.merchorderinappr_declineddate})</div>`
				notes = record.merchorderinappr_notes.replace(/(?:\r\n|\r|\n)/g, '<br>');
				notes = `<div style="margin-top: 5px; font-style: italic">${notes}</div>`
			} else if (record.merchorderinappr_isapproved=="1") {
				gambar = `<img src="images/xtionappr.png" width="20px" height="20px">`;
				authby = `<div><span style="font-weight: bold">${record.merchorderinappr_by}</span> (${record.merchorderinappr_date})</div>`
			} else {
