...
class PrintForm extends WebModule {
	function __construct() {
		// $logfilepath = __LOCALDB_DIR . "/output/logfile-merchquotout.txt";
	...
	public function LoadPage() {
		...
		try {
			...
			$stmt->execute([':merchquotout_id' => $id]);
			$rows  = $stmt->fetchall(\PDO::FETCH_ASSOC);
			$row = $rows[0];

			$this->merchquotout_id = $row['merchquotout_id'];
	...
	function getSqlHeader() {
		return "
			...
			from trn_merchquotout A where merchquotout_id = :merchquotout_id
		";
	}	
