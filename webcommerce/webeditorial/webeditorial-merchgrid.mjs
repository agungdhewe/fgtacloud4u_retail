...
const tbl_list = $('#pnl_editmerchgrid-tbl_list');
const txt_title = $('#pnl_editmerchgrid-title');
const pnl_control = $('#pnl_editmerchgrid-control');
const btn_removechecked  = $('#pnl_editmerchgrid-removechecked');
const btn_addrow = $('#pnl_editmerchgrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.editorial_title)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/merch-list`
		options.criteria['id'] = data.editorial_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editmerchform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editmerchform', () => {
		$ui.getPages().ITEMS['pnl_editmerchform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/merch-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editmerchform', ()=>{
		$ui.getPages().ITEMS['pnl_editmerchform'].handler.createnew(header_data)
	})	
}