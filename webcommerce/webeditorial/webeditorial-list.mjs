...
function grd_list_rowrender(tr) {
	...
	$(tr).find('td').each((i, td) => {
		...
		if (mapping=='editorial_title') {
			...
			td.innerHTML = `
				<div>
					<div class="inrow-id-block"><span style="margin-right: 3px">${record.editorial_id}</span>${status}</div>
				</div>
				<div style="font-weight: bold">${record.editorial_title}</div>
				<div style="font-style: italic">${record.editorial_preface}</div> 
			`;			
		}
