...
const btn_import = $('#pnl_list-btn_import');
...
export function init(param, fn_callback) {
	...
	btn_import.linkbutton({onClick: ()=>{ btn_import_click() }});
	
	fn_callback();
}

function btn_import_click() {
	$ui.getPages().show('pnl_editimport', ()=>{
		// $ui.getPages().ITEMS['pnl_editimport'].handler.setData();
