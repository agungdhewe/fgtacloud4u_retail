...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.merchquotout_id)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		...
		options.criteria['id'] = data.merchquotout_id
	}
...
function grd_list_rowrender(tr) {
	...
	$(tr).find('td').each((i, td) => {
		...
		if (mapping=='docauth_descr') {
			...
			var authby = "";
			var notes = "";			
			if (record.merchquotoutappr_isdeclined=="1") {
				gambar = `<img src="images/xtiondecl.png" width="20px" height="20px">`;
				authby = `<div><span style="font-weight: bold">${record.merchquotoutappr_declinedby}</span> (${record.merchquotoutappr_declineddate})</div>`
				notes = record.merchquotoutappr_notes.replace(/(?:\r\n|\r|\n)/g, '<br>');
				notes = `<div style="margin-top: 5px; font-style: italic">${notes}</div>`
			} else if (record.merchquotoutappr_isapproved=="1") {
				gambar = `<img src="images/xtionappr.png" width="20px" height="20px">`;
				authby = `<div><span style="font-weight: bold">${record.merchquotoutappr_by}</span> (${record.merchquotoutappr_date})</div>`
			} else {
